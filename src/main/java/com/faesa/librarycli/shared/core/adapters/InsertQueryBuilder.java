...
public class InsertQueryBuilder implements QueryBuilder {
    ...
    private String resolveQueryFields(StringBuilder builder, List<Field> fields) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            // check if the field is an entity reference, it can be done by checking if the field type is an instance of EntityClass
            if (field.getType().isAnnotationPresent(EntityClass.class)) {
                // if it is, then we need to get the type of the relationship by checking the annotation hasOne or hasMany and get the foreign key
                var annotation = field.getAnnotation(EntityClass.Column.class);
                var fieldName = annotation.name();

                // and finally we need to append the name of the field to the query
                builder.append(caseConverter.toSnakeCase(fieldName));
            } else {
                builder.append(caseConverter.toSnakeCase(field.getName()));
            }
